...
const DiscordAuth = ({onLoginSuccess}) => {
    ...
    return (<div>
        <button className="login-btn discord" onClick={requestOauth} disabled={isWaiting}>Login with Discord
        </button>
        {isWaiting && <p>Waiting for authentication...</p>}
        {/* {accountProfile && (<div>
            <p>Logged in as: {accountProfile.username}#{accountProfile.discriminator}</p>
        </div>)}*/}
    </div>);
};
